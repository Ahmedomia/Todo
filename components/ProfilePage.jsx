  ...
  const handleBackGroundChange = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${import.meta.env.VITE_API_URL}/users/${globalUser.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
    ...
    async function fetchProfile() {
      ...
      try {
        ...
        const blogsRes = await fetch(
          `${import.meta.env.VITE_API_URL}/blogs/user/${globalUser.id}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
    ...
    flex items-center justify-center text-white"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-2xl shadow-xl w-[550px] h-[430px] p-6 relative">
      ...
      <LogOutButton className="fixed top-4 right-4 z-50 bg-[#422f7d] p-2 rounded-full shadow-lg hover:bg-[#6840c6] transition-all transform hover:scale-110 active:scale-95 flex items-center justify-center text-white">
